...
defmodule Loadmaster.JobRunner do
  use GenServer
  ...
  def handle_cast(:run, %{job_id: job_id} = state) do
    job =
      Job
      |> Repo.get!(job_id)
      |> Repo.preload([:build, :image])

    repository = Repo.preload(job.build, :repository).repository

    job = Repo.update!(Job.changeset(job, %{state: "running", data: Map.put(job.data, :started_at, :os.system_time(:seconds))}))

    step_state = %StepState{repository: repository, job: job, build: job.build, git_remote: job.build.git_remote}
    |> step(:setup)
    |> step(:login)
    |> step(:clone)
    # |> step(:update_cache)
